...
하지만 현재 본과정에 들어온지 8개월 가량이 지났는데 선별과정(2월)부터 항상 인트라 알림 서비스에 불만이 있었다.<br/>
...
`등록 시간과 현재 시간을 비교해 시간 차이가 n초 이내일 경우`
...
- 단점
  1. 이전 호출에서 새로운 이벤트가 발생하였을때 다음 API호출에서 이전 이벤트 확인이 안돼 중복 전송 가능성이 크다.
  2. 위 문제를 해결하려 스택에 이전 이벤트의 PK를 갖고있자니 서버가 잠들거나 다운되면 이전 PK를 알 수 없으니 이미 전송된 이벤트가 두번 전송된다.
  3. 42 API 응답 오류인 경우 에러 핸들링이 귀찮아진다.

<br/>

`모든 이벤트를 DB에 저장하며 최신 이벤트와 비교하는 경우`
...
한가지 아쉬웠던 점은 본과정 워크스페이스 `42born2code`에는 이 앱을 42 Paris 관리자가 거절하여 등록을 못하였고, `허광남`멘토님의 도움으로 멘토님들이 운영하시는 `42seoul_cuckoo` 워크스페이스에 등록을 했다......😩
...
현재 하고있는 프로젝트가 종료되거나 도중에 시간이 나면 `TypeScript`를 적용해 전체 리팩토링을 해보려 한다 🤩<br/>
...
개발이 끝나고 3개월 후 현재 `224명`의 카뎃분들이 사용해 주시고 있다!!<br/>
